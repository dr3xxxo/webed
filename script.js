document.addEventListener('DOMContentLoaded', () => {
    ...
    const i18n = {
        ...
        en: {
            ...
            fallback_btn: "Standard Backup (Redirect)",
    ...
    if (heroConvertBtn) {
        heroConvertBtn.addEventListener('click', (e) => {
            e.preventDefault();
            if (heroConvertBtn.classList.contains('disabled')) {
                if (currentLang === 'fr') {
                    showNotification("Vous devez être connecté pour utiliser l'outil de conversion.", 'error');
                } else {
                    showNotification("You must be logged in to use the conversion tool.", 'error');
                }
            } else {
                if (heroView) heroView.style.display = 'none';
    ...
    if (heroScriptBtn) {
        heroScriptBtn.addEventListener('click', (e) => {
            ...
            if (heroScriptBtn.classList.contains('disabled')) {
                if (currentLang === 'fr') {
                    showNotification("Vous devez être connecté pour utiliser l'outil Script / macro.", 'error');
                } else {
                    showNotification("You must be logged in to use the Script / macro tool.", 'error');
                }
            } else {
                if (heroView) heroView.style.display = 'none';
    ...
    [settingsModal, loginModal, usageModal].forEach(modal => {
        if (!modal) return;
    ...
    function updateAuthState() {
        ...
        if (activeUser) {
            ...
            if (heroConvertBtn) {
                heroConvertBtn.classList.remove('disabled');
                // Remove lock icon completely
                const lockIcon = heroConvertBtn.querySelector('.lock-icon');
                if (lockIcon) lockIcon.style.display = 'none';

                // Update title
                heroConvertBtn.removeAttribute('title');
            }

            if (heroScriptBtn) {
                heroScriptBtn.classList.remove('disabled');
                const lockIcon = heroScriptBtn.querySelector('.lock-icon');
                if (lockIcon) lockIcon.style.display = 'none';
            ...
            if (heroConvertBtn) {
                heroConvertBtn.classList.add('disabled');
                // Show lock icon
                const lockIcon = heroConvertBtn.querySelector('.lock-icon');
                if (lockIcon) lockIcon.style.display = 'inline-block';

                // Update title
                heroConvertBtn.setAttribute('title', currentLang === 'fr' ? 'Connexion requise' : 'Login required');
            }
